...
4. **必要指导** —— 哪些领域约束、工具操作或判断依据是模型不能从项目自然推导的?流程长短按任务需要,不设最低步数。
5. **输出契约** —— 报告 / 文档 / message 需要包含哪些信息?只有机器消费或一致性确有要求时固定格式。
...
| 频次 | 每周 ≥ 1 次 | 每周 ≥ 1 次(同) |
| 流程步骤 | 保留有用约束,不设步数配额 | 分类 / 扫现状 / 推荐 / 失败恢复,按实际请求选取 |
| 输出格式 | 必要信息完整 | 简短建议,按需提供 status / next / recover |
...
- ❌ 不把存在 artifact 当作已批准 —— 读取必要的 status / scope / blocker,不做全面内容审查
